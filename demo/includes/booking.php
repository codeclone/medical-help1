...
if( $_SERVER['REQUEST_METHOD'] == 'POST' ) {
    if( $_POST['booking_name'] != '' AND $_POST['booking_email'] != '' AND $_POST['booking_phone'] != '' AND $_POST['booking_date'] != '' ) {

        $name = $_POST['booking_name'];
        $email = $_POST['booking_email'];
        $phone = $_POST['booking_phone'];
        $date = $_POST['booking_date'];
        $post = $_POST['depertment_post'];
        ...
        if( $botcheck == '' ) {
            ...
            $date = isset($date) ? "Booking date: $date<br><br>" : '';
            $post = isset($post) ? "Depertment Post: $post<br><br>" : '';

            $referrer = $_SERVER['HTTP_REFERER'] ? '<br><br><br>This Form was submitted from: ' . $_SERVER['HTTP_REFERER'] : '';

            $body = "$name $email $phone $date $post $message $referrer";
